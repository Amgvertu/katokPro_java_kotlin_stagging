...
@HiltViewModel
class MonitoringUsersViewModel @Inject constructor(
    private val repository: MonitoringRepository
) : ViewModel() {

    private val _stats = MutableLiveData<UserStatsResponse?>(null)
    val stats: LiveData<UserStatsResponse?> = _stats
    ...
    private val _selectedPositions = MutableLiveData<List<String>>(emptyList())
    val selectedPositions: LiveData<List<String>> = _selectedPositions
    ...
    fun setPositions(positions: List<String>) {
        _selectedPositions.value = positions
        loadStatistics()
    }
    ...
    fun loadStatistics() {
        viewModelScope.launch {
            ...
            _error.value = null
            val result = repository.getUsersStatistics(
                cityIds = _selectedCityIds.value?.takeIf { it.isNotEmpty() },
                dateFrom = _dateFrom.value,
                dateTo = _dateTo.value,
                positions = _selectedPositions.value?.takeIf { it.isNotEmpty() }
            )
            when (result) {
